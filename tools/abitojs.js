...
const generate = () => {
  ...
  Object.values(SUPPORTED_NETWORKS).map(network => {
    ...
    const synthContracts = snx.getSynths({ network }).reduce((memo, { name, subclass }) => {
      ...
      // @TODO kev: proxy contracts dont appear to be working :O
      memo[name] = { target: `Proxy${name}`, source: subclass || 'Synth' };
      return memo;
    }, {});
...
const generateJSFile = (contractName, network, target, functions, source) => {
  const content = `
    ...
    function ${contractName}(contractSettings) {
      ...
      const address = this.contractSettings.addressList['${target}']
...
const generateFunctionStr = (abiFn, source) => {
  ...
  const methodParamsStr = getFnParams(params);
  if (!abiFn.constant) {
  ...
  if (abiFn.constant) {
    // @kev Need _isConstant when proxied method, weird...send PR to tronweb?
    return `${jsdoc}
      this.${abiFn.name} = async (${paramsStr}) => {
        return await this.contract.${abiFn.name}(${methodParamsStr}).call({ _isConstant: true })
      };
    `;
  }
