   ...
   **/
      this.lastRateUpdateTimesForCurrencies = async (currencyKeys) => {
        return await this.contract.lastRateUpdateTimesForCurrencies(currencyKeys).call({ _isConstant: true })
      };
   ...
   **/
    this.setInversePricing = async (currencyKey, entryPoint, upperLimit, lowerLimit, freeze, freezeAtUpperLimit, txParams) => {
      txParams = txParams || {};
      const txHash = await this.contract.setInversePricing(currencyKey, entryPoint, upperLimit, lowerLimit, freeze, freezeAtUpperLimit).send(txParams);
      return { hash: txHash };
    };
