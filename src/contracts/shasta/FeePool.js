    ...
    import abi from '../../../lib/abis/shasta/FeePool';
   ...
   **/
      this.effectiveDebtRatioForPeriod = async (account, period) => {
        return await this.contract.effectiveDebtRatioForPeriod(account, period).call({ _isConstant: true })
      };
   ...
   **/
    this.appendAccountIssuanceRecord = async (account, debtRatio, debtEntryIndex, txParams) => {
      txParams = txParams || {};
      const txHash = await this.contract.appendAccountIssuanceRecord(account, debtRatio, debtEntryIndex).send(txParams);
      return { hash: txHash };
    };
   ...
   **/
    this.importFeePeriod = async (feePeriodIndex, feePeriodId, startingDebtIndex, startTime, feesToDistribute, feesClaimed, rewardsToDistribute, rewardsClaimed, txParams) => {
      txParams = txParams || {};
      const txHash = await this.contract.importFeePeriod(feePeriodIndex, feePeriodId, startingDebtIndex, startTime, feesToDistribute, feesClaimed, rewardsToDistribute, rewardsClaimed).send(txParams);
      return { hash: txHash };
    };
