...
<br>Payable (to enter TRX amount set txParams.value)
   ...
   **/
    this.updatePrices = async (newEthPrice, newSynthetixPrice, timeSent, txParams) => {
      txParams = txParams || {};
      const txHash = await this.contract.updatePrices(newEthPrice, newSynthetixPrice, timeSent).send(txParams);
      return { hash: txHash };
    };
   ...
   **/
    this.exchangeSynthsForSynthetixAtRate = async (synthAmount, guaranteedRate, txParams) => {
      txParams = txParams || {};
      const txHash = await this.contract.exchangeSynthsForSynthetixAtRate(synthAmount, guaranteedRate).send(txParams);
      return { hash: txHash };
    };
...
<br>Payable (to enter TRX amount set txParams.value)
   ...
   **/
    this.exchangeEtherForSynthetixAtRate = async (guaranteedEtherRate, guaranteedSynthetixRate, txParams) => {
      txParams = txParams || {};
      const txHash = await this.contract.exchangeEtherForSynthetixAtRate(guaranteedEtherRate, guaranteedSynthetixRate).send(txParams);
      return { hash: txHash };
    };
